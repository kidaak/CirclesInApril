package gmail.bendavisnc.GeoPicasso
...
//    .doOnCompleted(this.savePng)
  ...
  def nextShape(basedOnPreviousShape: Option[ShapeModel]): Option[ShapeModel] = {
    ...
    def nextRightShape(): Option[ShapeModel] = {
      val ROUNDING_WEIRDNESS = 0.000001
      if (lastShape.cx + lastShape.r * 3 > 1 + ROUNDING_WEIRDNESS)
//      if (lastShape.cx + lastShape.r >= 1)
        return None
...
//    val wholeFilename = "%s%s.svg".format(this.docFilename, aUniqueTag())
  ...
  def savePng(): Unit = {
    ...
    var svgOutFile: FileOutputStream = null
    var svgOut: TranscoderOutput = null
    GeoPicassoRx.outputStream match {
      case null => {
        svgOutFile = new FileOutputStream("%s%s.png".format(this.docFilename, this.docTag))
        svgOut= new TranscoderOutput(svgOutFile)
      }
      case _ => {
        svgOut= new TranscoderOutput(GeoPicassoRx.outputStream)
      }
    }
...
//    stripOutside()
    ...
    GeoPicassoRx.outputStream match {
      case null => {
        svgOutFile = new FileOutputStream(s"generated/${contextInfo.name}.jpg")
        svgOut= new TranscoderOutput(svgOutFile)
      }
      case _ => {
        svgOut= new TranscoderOutput(GeoPicassoRx.outputStream)
    ...
    if (svgOutFile != null) svgOutFile.flush()
    if (svgOutFile != null) svgOutFile.close()
//    def writeExifData() = {
//      val exifDataSource = new TiffOutputSet();
////      val metadata = Imaging.getMetadata(GeoPicassoRx.outputStream.toByteArray).asInstanceOf[JpegImageMetadata]
////      val exifDataSource = metadata.getExif().getOutputSet()
//      val exifData: TiffOutputDirectory = exifDataSource.getOrCreateExifDirectory()
////      val exifData: TiffOutputDirectory = exifDataSource.getOrCreateRootDirectory()
//      exifData.add(ExifTagConstants.EXIF_TAG_USER_COMMENT, contextInfo.jsonRef.get.toString())
//      exifData.add(ExifTagConstants.EXIF_TAG_OWNER_NAME, "bendavisnc")
////      exifDataSource.
//      new ExifRewriter().updateExifMetadataLossless(GeoPicassoRx.outputStream.toByteArray, GeoPicassoRx.outputStream, exifDataSource)
//      IoUtils.closeQuietly(true, GeoPicassoRx.outputStream);
//    }
//    writeExifData()
    // TODO: really wish I could get jpg tag writing to work
  ...
  def doOnStart(): Unit = {
    ...
    def createOutputSvg(): Unit = {
      ...
      def backgroundHack(): Element = {
        ...
        rect.attr("fill", this.contextInfo.background.color)
        rect.attr("fill-opacity", this.contextInfo.background.opacity.toString)
        return rect
      }
...
//                  val background: String,
  ...
  def this(
    ...
    width: Int,
    height: Int) = {
    this(name, background, shapesAlongX, shapesSequence, fillStyles, strokeStyles, scale, left, top, width, height, Some[JSONObject](null))
  }
}
...
//  object fromSvg {
//    lazy val svgDoc = Jsoup.parse(scala.io.Source.fromFile("minimizedOutputTemplate.svg").mkString, "", Parser.xmlParser())
...
//  }
  ...
  object fromJson {
    def getOffsetFrom(someVal: AnyRef, offsetType: String): Option[Float] = {
      def calcPercentOffset(fromPercentVal: Float, length: Float, scale: Float, radius: Float): Float = {
        val desiredOffset = length * (fromPercentVal / 100)
        val di = desiredOffset + scale * radius - (scale * length / 2)
        return di
      }
      someVal match {
        case s: String if s.length == 0 => None
        case s: String if s.charAt(s.length - 1) == '%' => Some(calcPercentOffset(s.substring(0, s.length - 1).toFloat
                                                             , if (offsetType == "left") width else height,
                                                             scale,
                                                             height / 2
                                                             ))
        case s: String => {
    ...
    def background = {
        val backgroundSerialized = jsonSource.get.getJSONObject("background")
        new FillStyle(backgroundSerialized.getString("color"), backgroundSerialized.getDouble("opacity").toFloat)
    }
    def shapesAlongX = jsonSource.get.getInt("shapesAlongX")
    // shape elements
    def shapesSerialized = jsonSource.get.getJSONArray("shapes")
    ...
    // fills
    def fillsSerialized = jsonSource.get.getJSONArray("fills")
    ...
    // strokes
    def strokesSerialized = jsonSource.get.getJSONArray("strokes")
    ...
    def left: Option[Float] = getOffsetFrom(jsonSource.get.get("left"), "left")
    def top: Option[Float] = getOffsetFrom(jsonSource.get.get("top"), "top")
...
//    new testing
  ...
  def render(what: JSONObject): Array[Byte] = {
    outputStream = new ByteArrayOutputStream()
    new GeoPicassoRx(ContextInfo.generateDefault(what))
